...
class DebitNote extends React.Component {
    constructor(props) {
        super(props);
        this.state = debitnote_state();
        if (props.public_id) {
            this.state.debitnote_public_id = props.public_id;
            // this.load_debitnote(this.state.debitnote_public_id);
        }
        window.debitnotereactelement = this;
        this.load_data();
        if (props.public_id) {
            this.load_debitnote(props.public_id);
        }
    }
    ...
    load_data = (onsuccess) => {
        ...
        jQuery.ajax({
            type: 'get',
            url: window.urlfor_debitcreditnotepage_init_data,
            data: {},
            dataType: 'json',
    ...
    load_debitnote = (debitnote_public_id) => {
        let _this = this;
        let spinner = jQuery('<div class=""><i class="fa fa-spinner fa-spin"></i> Loading debit note #' + debitnote_public_id + '</div>');
        let loader = window.modal_alert();
        loader.append_html(spinner);
        jQuery.ajax({
            type: 'get',
            url: window.urlfor_get_debotnote_info,
            data: {
                public_id: debitnote_public_id
            },
            dataType: 'json',
            success: function (resp) {
                if (resp.success) {
                    ...
                    _this.setState({
                        debitnote_public_id: resp.data.debitnote_dict.public_id,
                        currency: Object.assign(_this.state.currency, resp.data.debitnote_dict.currency),
                        currency_conversion: Object.assign(_this.state.currency_conversion, resp.data.debitnote_dict.currency_conversion),
                        to_who: resp.data.debitnote_dict.to_who,
                        to_address_dict: Object.assign(_this.state.to_address_dict, resp.data.debitnote_dict.to_address_dict),
                        date: resp.data.debitnote_dict.date,
                        mawb: Object.assign(_this.state.mawb, resp.data.debitnote_dict.mawb),
                        hawb: Object.assign(_this.state.hawb, resp.data.debitnote_dict.hawb),
                        charges_list: resp.data.debitnote_dict.charges_list.map(charge => {
                            charge.errors = [];
                            charge.form_errors = {};
                            return charge;
                        })
    ...
    address_selection = (mawb_load_ajax_resp, callback) => {
        ...
        let popup_content = jQuery('<div class="card">' +
            '<div class="card-header bg-secondary">MAWB: ' + resp.data.mawb_dict.public_id + ' ' +
            ...
            '<input type="radio" name="to_party" value="mawb_consignee"> MAWB consignee address ('
        ...
        jQuery.ajax({
            ...
            success: function (resp) {
                if (resp.success) {
                    ...
                    if (resp.data.mawb_dict.has_debitnote) {
                        loader.append_html('<div class="text-warning">Debit note exists.</div>');
                        let debitnoteselection = jQuery('<div class="form-group"><label>Select a debit not to load</label><select class="form-control"><option' +
                            ' value="">--</option>' + resp.data.mawb_dict.debitnote_list.map(info => '<option value="' + info.public_id + '">#' + info.public_id + ' ' + info.to_who + '</option>').join('') + '</select></div><div class="form-group"><button type="button">or create another debit note</button></div>');
                        loader.append_html(debitnoteselection);
                        loader.upon_ok = function () {
                            const public_id = debitnoteselection.find('select').val();
                            if (public_id) {
                                loader.close();
                                _this.load_debitnote(public_id);
                            } else {
                                loader.set_footer_msg('<div class="text-danger">Please select</div>');
                            }
                        };
                        debitnoteselection.find('button').click(function (event) {
                            loader.close();
    ...
    mawb_selection = (callback) => {
        ...
        let form = jQuery('<div class="form-group"><label>Select MAWB</label><select id="mawb_public_id" class="form-control"><option value="">--</option>' + mawb_options + '</select></div>');
        ...
        modal.upon_ok = () => {
            ...
            if (mawb_public_id) {
                ...
                modal.set_footer_msg('<div class="text-danger">insert MAWB ID in the input field</div>');
            }
        };
    };
    ...
    save = () => {
        ...
        jQuery.ajax({
            type: 'POST',
            url: window.urlfor_debit_note_save,
            data: {
                debitnote_public_id: _this.state.debitnote_public_id,
            ...
            success: function (resp) {
                if (resp.success) {
                    _this.setState({
                        msg: resp.msg,
                        debitnote_public_id: resp.data.public_id
                    });
    ...
    render() {
        return <DebitNoteForm context={this}/>;
    }
